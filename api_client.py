...
class FutbolAPI:
    ...
    def _istek_yap(self, endpoint: str, params: Optional[Dict] = None) -> Optional[Dict]:
...
if __name__ == "__main__":
    ...
    for futbolcu in futbolcular[:5]:  # İlk 5 sonucu göster
        print(f"- {futbolcu.get('name')} ({futbolcu.get('team')}, {futbolcu.get('nationality')})")
    ...
    for mac in maclar[:3]:  # İlk 3 sonucu göster
        print(f"- {mac.get('home_team')} vs {mac.get('away_team')} - {mac.get('competition')}")
