...
class MatchPredictor:
    ...
    def get_team_form(
        ...
        try:
            ...
            return {
                ...
                'clean_sheet_percent': round((clean_sheets / total_matches) * 100, 1) if total_matches > 0 else 0,
                'failed_to_score': failed_to_score,
                'failed_to_score_percent': round((failed_to_score / total_matches) * 100, 1) if total_matches > 0 else 0
            }
            ...
            return {
                ...
                'avg_goals_for': 0, 'avg_goals_against': 0,
                'clean_sheets': 0, 'failed_to_score': 0,
                'clean_sheet_percent': 0, 'failed_to_score_percent': 0
            }
    ...
    def _simple_prediction(self, home_form: dict, away_form: dict) -> dict:
        ...
        try:
            ...
            # 5+ gol olasılığı
            high_scoring_prob = self._calculate_high_scoring_prob(home_expected, away_expected)
    ...
    def _calculate_high_scoring_prob(self, home_goals: float, away_goals: float) -> float:
    ...
    def _calculate_outcome_probabilities(self, home_goals: float, away_goals: float) -> Tuple[float, float, float]:
    ...
    def prepare_match_features(
        ...
        try:
            # Takım formlarını al
            home_form = self.get_team_form(home_team_id, match_date, league_id=league_id)
            away_form = self.get_team_form(away_team_id, match_date, league_id=league_id)
            
            # Temel özellikler
            features = [
                home_form['avg_goals_for'],
                home_form['avg_goals_against'],
                home_form['points'] / (3 * (home_form['wins'] + home_form['draws'] + home_form['losses'] or 1)),
                away_form['avg_goals_for'],
                away_form['avg_goals_against'],
                away_form['points'] / (3 * (away_form['wins'] + away_form['draws'] + away_form['losses'] or 1)),
    ...
    def predict_match(self, home_team_id: int, away_team_id: int, match_date: datetime = None, league_id: int = None) -> Dict:
        """
        ...
        try:
            ...
            features = self.prepare_match_features(home_team_id, away_team_id, match_date, league_id)
            if features is None:
            ...
            # 5+ gol olasılığı
            high_scoring_prob = self._calculate_high_scoring_prob(home_goals, away_goals)
            
            # Maç sonucu olasılıkları
            home_win_prob, draw_prob, away_win_prob = self._calculate_outcome_probabilities(home_goals, away_goals)
            ...
            return {
                ...
                'model_used': f"league_{self.league_id}" if self.league_id else 'global'
            }
            
        except Exception as e:
            logger.error(f"Maç tahmini yapılırken hata (Ev: {home_team_id}, Deplasman: {away_team_id}): {str(e)}")
            return {
                'error': 'Tahmin yapılırken bir hata oluştu',
                'details': str(e)
            }
...
class MatchPredictor:
    ...
    def _initialize_model(self) -> None:
        ...
        try:
            ...
            if os.path.exists(self.model_path) and os.path.exists(self.scaler_path):
                self.model = load(self.model_path)
                self.scaler = load(self.scaler_path)
                logger.info(f"Önceden eğitilmiş model ve scaler yüklendi: {self.model_path}")
            else:
                self._create_new_model()
        except Exception as e:
            logger.error(f"Model yüklenirken hata oluştu: {str(e)}")
        ...
        for match in matches:
            try:
                ...
                prediction = self.predict_match(
                    match['home_team_id'],
                    match['away_team_id'],
                    datetime.strptime(match['match_date'], '%Y-%m-%d') if isinstance(match['match_date'], str) else match['match_date']
                )
                
                # Beraberlik olasılığını kontrol et
                if prediction and prediction.get('match_prediction', {}).get('draw_prob', 0) >= min_draw_prob:
                    high_draw_matches.append({
                        'match': match,
                        'prediction': prediction,
                        'draw_probability': prediction['match_prediction']['draw_prob']
                    })
                    
            except Exception as e:
                logger.error(f"Berabere maç analizinde hata (Maç ID: {match.get('id')}): {str(e)}")
                continue
                
        # Beraberlik olasılığına göre sırala (yüksekten düşüğe)
        return sorted(high_draw_matches, key=lambda x: x['draw_probability'], reverse=True)
    ...
    def get_team_form(self, team_id, match_date, matches_back=5):
        ...
        try:
            ...
            if not matches:
                return {'wins': 0, 'draws': 0, 'losses': 0, 'goals_for': 0, 'goals_against': 0, 'form': []}
            ...
            return {
                ...
                'avg_goals_against': round(goals_against / len(matches), 2) if matches else 0
            }
    ...
    def prepare_match_data(self, home_team_id, away_team_id, match_date, matches_back=5):
    ...
    def calculate_high_scoring_probability(self, home_team_id: int, away_team_id: int, match_date: datetime = None) -> float:
        """
        ...
        try:
            ...
            home_goals_avg = self._get_average_goals(home_team_id, match_date, is_home=True)
            away_goals_avg = self._get_average_goals(away_team_id, match_date, is_home=False)
            ...
            prob = 1 - sum([(total_avg ** k * np.exp(-total_avg)) / np.math.factorial(k) for k in range(5)])
    ...
    def _get_average_goals(self, team_id: int, match_date: datetime, is_home: bool) -> float:
    ...
    def train_model(self, matches=None, league_id: int = None):
        ...
        try:
            ...
            logger.info(f"Model eğitimi tamamlandı. Eğitim skoru: {train_score:.4f}, Test skoru: {test_score:.4f}")
