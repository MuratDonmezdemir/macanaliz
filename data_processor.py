...
from models import Team, Match, Player, InjuryReport
from app import db
...
class DataProcessor:
    ...
    def _get_recent_match_stats(self, team, limit=5):
        ...
        ).filter(Match.played == True).order_by(Match.match_date.desc()).limit(limit).all()
    ...
    def _get_player_stats(self, team):
        ...
        # Player age analysis
        experienced_players = sum(1 for player in players if player.age and player.age >= 28)
        young_prospects = sum(1 for player in players if player.age and player.age <= 21)
        
        # Top scorer
        top_scorer_goals = max(player.goals_scored for player in players) if players else 0
    ...
    def _get_injury_stats(self, team):
        ...
        # Current injuries
        current_injuries = InjuryReport.query.join(Player).filter(
            Player.team_id == team.id,
            InjuryReport.status == 'injured'
        ).all()
        
        # Key player injuries (high skill rating players)
        key_players_injured = sum(
            1 for injury in current_injuries 
            if injury.player.skill_rating >= 70
        )
        ...
        return {
            ...
            'injury_severity_avg': sum(injury.severity for injury in current_injuries) / len(current_injuries) if current_injuries else 0
        }
    ...
    def _calculate_performance_trends(self, team):
        ...
        defensive_trend = older_stats['goals_against'] - recent_stats['goals_against']  # Lower goals against is better
    ...
    def _get_head_to_head_stats(self, home_team, away_team):
        ...
        ).filter(Match.played == True).order_by(Match.match_date.desc()).limit(10).all()
        ...
        stats['home_team_dominance'] = total_points_home / total_points if total_points > 0 else 0.5
    ...
    def calculate_team_statistics(self, team):
        ...
        if all_matches:
            ...
            stats.update({
                ...
                'avg_goals_per_match': goals_for / total_matches if total_matches > 0 else 0,
