...
import datetime
from sqlalchemy import Column, Integer, String, DateTime, ForeignKey, Boolean, Text, func, Float, Numeric, Index, CheckConstraint, event
from sqlalchemy.dialects.postgresql import JSONB, ARRAY
from sqlalchemy.orm import relationship, backref, validates, object_session, Session
from sqlalchemy.ext.hybrid import hybrid_property
from sqlalchemy.sql import case, and_, or_

from .base import BaseModel
from app.extensions import db
from app.utils.enums import MatchStatus, MatchEventType, CardType, WeatherCondition
...
class Match(BaseModel):
    ...
    Attributes:
        ...
        competition_id: Yarışma/Lig ID'si
        match_date: Maç tarihi ve saati
        status: Maç durumu (SCHEDULED, FINISHED vb.)
        home_goals: Ev sahibi takımın attığı goller
        away_goals: Deplasman takımının attığı goller
        home_ht_goals: İlk yarı ev sahibi golleri
        away_ht_goals: İlk yarı deplasman golleri
        home_ft_goals: İkinci yarı ev sahibi golleri
        away_ft_goals: İkinci yarı deplasman golleri
        home_et_goals: Uzatma devresi ev sahibi golleri
        away_et_goals: Uzatma devresi deplasman golleri
        home_penalties: Penaltı atışları ev sahibi golleri
        away_penalties: Penaltı atışları deplasman golleri
        home_stats: Ev sahibi takım istatistikleri (JSON)
        away_stats: Deplasman takımı istatistikleri (JSON)
        weather: Hava durumu bilgileri (JSON)
        attendance: Seyirci sayısı
        referee: Hakem bilgileri
        venue: Stadyum bilgileri
        round: Lig sıralaması
        matchday: Hafta bilgisi
        is_derby: Derbi maçı mı?
        is_tv: Televizyonda yayınlanacak mı?
        broadcasters: Yayıncı kuruluşlar (JSON)
        odds: Bahis oranları (JSON)
    """
    ...
    goals = db.relationship('MatchGoal', back_populates='match', cascade='all, delete-orphan')
    cards = db.relationship('MatchCard', back_populates='match', cascade='all, delete-orphan')
    substitutions = db.relationship('MatchSubstitution', back_populates='match', cascade='all, delete-orphan')
    statistics = db.relationship('MatchStatistics', back_populates='match', cascade='all, delete-orphan')
